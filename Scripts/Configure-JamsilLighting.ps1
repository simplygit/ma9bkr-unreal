param(
    ...
    [string]$PythonScriptPath = "F:\workspace_f\KR_unreal\Scripts\configure_jamsil_lighting.py"
)

$runningEditor = Get-Process UnrealEditor -ErrorAction SilentlyContinue
if ($runningEditor) {
    Write-Error "Unreal Editor is currently running. Close the editor before configuring Jamsil lighting."
    exit 1
}
...
if (!(Test-Path -LiteralPath $PythonScriptPath)) {
    Write-Error "Python lighting script not found: $PythonScriptPath"
    exit 1
}
...
if ($exitCode -ne 0) {
    Write-Error "Jamsil lighting configuration failed with exit code $exitCode."
    exit $exitCode
}

Write-Host "Jamsil lighting configuration completed successfully."
